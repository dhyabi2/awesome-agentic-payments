...
- [Vend](https://paypercall.dev) — live pay-per-call data APIs for AI agents settled in **native Nano (XNO)** via the x402 `exact` scheme; fee-free, instant, no issuer. Extraction, web search, IP geo, domain intel, Nano account info; self-facilitates `PAYMENT-SIGNATURE`, so a spec x402 client pays directly with no stablecoin and no facilitator in the path. Manifest [`/.well-known/x402`](https://extract.paypercall.dev/.well-known/x402), [OpenAPI](https://extract.paypercall.dev/openapi.json), 0.0001–0.0005 XNO per call.
